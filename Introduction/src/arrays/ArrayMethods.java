...
public class ArrayMethods {
	...
	public static void main(String[] args) {
		...
		int[] test = {5,7,8,9,10,20};
		int[] test2 = {20,10,9,8,7,5};
		//print(searchUnsorted(test,11));
		//print(countDifferences(test, test2));
	    ...
	    private static void printArray(int[] generateDistinctItemsList) {
		// TODO Auto-generated method stub
		for(int i = 0; i<generateDistinctItemsList.length; i++)
		{
			System.out.println(generateDistinctItemsList[i]);
		}
	}
	     ...
	     * */
//	    	Arrays.sort(sortedArrayToSearch);
...
//	    	}
	    ...
	    public static int countDifferences(int[] array1, int[] array2){
	    	...
	    	{
	    			if (searchUnsorted(array2, array1[i]) == -1)
	    			{
	    				difference++;
	    			}
	    			else if(searchUnsorted(array2, array1[i]) != searchUnsorted(array1, array1[i]))
	    			{
	    				difference++;
	    			}
	    	}
	    ...
	    public static int[] generateDistinctItemsList(int n){
	    	...
	    	for(int i = 0; i<numArray.length;i++)
	    	{
	    		...
	    		if(searchUnsorted(numArray, num) == -1)
	    		{
	    			numArray[i] = num;
	    		}
	    		else
	    		{
	    			int num2 = (int) (Math.random() * 2 * n + 1);
	    			if(searchUnsorted(numArray, num2) > -1)
    				{
    					equal = true;
