...
public class ArrayHelp {
	...
	public static int searchUnsorted(int[] arrayToSearch, int key) {
		 ...
		 * int corresponding to the index of a key, if it is in the array if the
		 * key is not in the array, this method returns -1
		 */
	...
	public static int searchSorted(int[] sortedArrayToSearch, int key) {
		/**
		 * this method is exactly like the one above, except the parameter
		 * sortedArrayToSearch will always be sorted in DESCENDING order. Again
		 * return the index of the key or return -1 if the key is not in the
		 * array
		 * 
		 * Note: You should attempt to write a method that is more efficient
		 * that searchUnsorted
		 */
		if (sortedArrayToSearch.length == 0) {
			return -1;
		}

		int low = 0, high = sortedArrayToSearch.length;
		while (high >= low) {
			int mid = low + (high - low) / 2;
			if (sortedArrayToSearch[mid] < key) {
				high = mid - 1;
    ...
    public static int longestConsecutiveSequence(int[] array1){
         ...
         * */
        int longest = 1, consec = 1;
        for (int i = 1; i != array1.length; i++) {
            if (array1[i] == array1[i - 1] + 1) {
                consec++;
            } else {
                longest = consec > longest ? consec : longest;
                consec = 1;
            }
        }
